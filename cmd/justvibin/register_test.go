...
func TestRegisterCmdAlreadyRegistered(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(tmp, ".justvibin"), []byte(`{"name":"myapp"}`), 0644); err != nil {
...
func TestRegisterCmdDetectsDjango(t *testing.T) {
	...
	if registeredTemplate != "django-hypermedia" {
		t.Fatalf("expected django-hypermedia, got %s", registeredTemplate)
	}
}
