...
type = "static"
`
	if err := os.WriteFile(filepath.Join(templateDir, "justvibin.toml"), []byte(manifest), 0644); err != nil {
		t.Fatalf("write: %v", err)
	}

	configDir := t.TempDir()
	t.Setenv("XDG_CONFIG_HOME", configDir)
	if err := os.Symlink(templateDir, filepath.Join(configDir, "justvibin", "templates", "mytemplate")); err != nil {
		if err := os.MkdirAll(filepath.Join(configDir, "justvibin", "templates"), 0755); err != nil {
			t.Fatalf("mkdir: %v", err)
		}
		src, _ := os.ReadFile(filepath.Join(templateDir, "justvibin.toml"))
		destDir := filepath.Join(configDir, "justvibin", "templates", "mytemplate")
		if err := os.MkdirAll(destDir, 0755); err != nil {
			t.Fatalf("mkdir: %v", err)
		}
		if err := os.WriteFile(filepath.Join(destDir, "justvibin.toml"), src, 0644); err != nil {
			t.Fatalf("write: %v", err)
		}
	}
...
func TestHeadlessInstallListOfficial(t *testing.T) {
	...
	rootCmd.SetOut(stdout)
	rootCmd.SetErr(stderr)
	rootCmd.SetArgs([]string{"install", "--list-official"})
	if err := rootCmd.Execute(); err != nil {
...
func TestHeadlessQuietFlag(t *testing.T) {
	...
	rootCmd.SetOut(stdout)
	rootCmd.SetErr(stderr)
	rootCmd.SetArgs([]string{"--quiet", "templates"})
	_ = rootCmd.Execute()
}
...
func TestHeadlessNoColorFlag(t *testing.T) {
	...
	rootCmd.SetOut(stdout)
	rootCmd.SetErr(stderr)
	rootCmd.SetArgs([]string{"--no-color", "templates"})
	_ = rootCmd.Execute()
}
