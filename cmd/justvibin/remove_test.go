...
func resetRemoveFlags(t *testing.T) {
	t.Helper()
	resetRootFlags(t)
	if f := removeCmd.Flags().Lookup("files"); f != nil {
		_ = f.Value.Set("false")
		f.Changed = false
	}
	if f := removeCmd.Flags().Lookup("yes"); f != nil {
		_ = f.Value.Set("false")
		f.Changed = false
	}
}
...
func TestRemoveCmdSuccess(t *testing.T) {
	...
	rootCmd.SetOut(stdout)
	rootCmd.SetErr(stderr)
	rootCmd.SetArgs([]string{"remove", "myapp"})
	if err := rootCmd.Execute(); err != nil {
...
func TestRemoveCmdAlias(t *testing.T) {
	...
	projectDir := t.TempDir()
	if err := os.WriteFile(filepath.Join(projectDir, ".justvibin"), []byte(`{"name":"myapp"}`), 0644); err != nil {
		t.Fatalf("write: %v", err)
	}
	...
	rootCmd.SetOut(stdout)
	rootCmd.SetErr(stderr)
	rootCmd.SetArgs([]string{"rm", "myapp"})
	if err := rootCmd.Execute(); err != nil {
		t.Fatalf("expected exit 0, got %v", err)
	}
	if !strings.Contains(stdout.String(), "Removed: myapp") {
...
func TestRemoveCmdWithFilesFlag(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(projectDir, ".justvibin"), []byte(`{"name":"myapp","port":59999}`), 0644); err != nil {
		t.Fatalf("write: %v", err)
	}
	...
	rootCmd.SetOut(stdout)
	rootCmd.SetErr(stderr)
	rootCmd.SetArgs([]string{"remove", "myapp", "--files", "-y"})
	if err := rootCmd.Execute(); err != nil {
		t.Fatalf("expected exit 0, got %v", err)
	}
...
func TestRemoveCmdWithFilesFlagCancelled(t *testing.T) {
	...
	rootCmd.SetErr(stderr)
	rootCmd.SetIn(stdin)
	rootCmd.SetArgs([]string{"remove", "myapp", "--files"})
	if err := rootCmd.Execute(); err != nil {
		t.Fatalf("expected exit 0, got %v", err)
	}
