...
func (c setupCommand) run(ctx context.Context, args []string, console *ui.UI, logger *logging.Logger, styled bool) int {
	console.PrintHeader("justvibin", version.Version)
	checkOnly := false
	...
	logger.Info("Setting up justvibin...")
	...
	if installTemplates {
		if err := c.offerOfficialTemplates(ctx, console, logger, styled); err != nil {
			logger.Error("Failed to install official templates")
			return 1
		}
	}

	logger.Success("Setup complete!")
	logger.Info("Create your first project: justvibin new myproject")
	return 0
}
...
func (c setupCommand) checkAllDependencies(ctx context.Context, logger *logging.Logger) bool {
	...
	c.checkDependency(ctx, logger, "gum", "gum", false)
