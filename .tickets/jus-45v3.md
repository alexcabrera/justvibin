...
---
# Implement 'justvibin register' command

Implement the register command for adding existing directories to justvibin management.

## Usage

```
justvibin register [name]           # Register current directory
justvibin register --template <t>   # Specify template type
```

## Use Case

For projects created outside justvibin that want to use its serving features:
...
cmd_register() {
    ...
    if [[ -f "$project_dir/.justvibin" ]]; then
    ...
    log_info "Start: justvibin start"
}
