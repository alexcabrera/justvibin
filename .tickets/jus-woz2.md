...
Migrate the 'justvibin install' command from manual arg parsing to Cobra subcommand.

## Background

justvibin is a CLI for scaffolding web projects. The 'install' command installs template plugins from git repositories into ~/.config/justvibin/templates/.

## Current State

File: cmd/justvibin/install.go
...
1. Create cmd/justvibin/cmd_install.go with Cobra command:
...
var installCmd = &cobra.Command{
    ...
    Long:  "Clone a template repository and install it as a plugin. Templates must contain a justvibin.toml manifest file.",
...
```
Examples:
  justvibin install https://github.com/user/template.git    # Install from URL
  justvibin install https://github.com/user/repo.git -n my  # Install with custom name
  justvibin install --list-official                         # Show official templates
```

## Acceptance Criteria

- [ ] justvibin install --help shows all flags with descriptions
- [ ] justvibin install <url> installs template
- [ ] justvibin install <url> --name custom works
- [ ] justvibin install --list-official shows official templates
- [ ] justvibin install (no url, no --list-official) shows error
