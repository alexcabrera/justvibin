...
---
# Implement 'justvibin uninstall' command
...
```
justvibin uninstall <name>   # Remove template
justvibin uninstall --all    # Remove all templates (with confirmation)
```
...
cmd_uninstall() {
    ...
    if [[ "$name" == "--all" ]]; then
        if confirm "Remove ALL installed templates?"; then
            rm -rf "$JUSTVIBIN_TEMPLATES_DIR"/*
            log_success "All templates removed"
        fi
        return
    fi
    
    local template_dir="$JUSTVIBIN_TEMPLATES_DIR/$name"
    ...
    using_projects=$(jq -r --arg t "$name" \
        ...
        "$JUSTVIBIN_PROJECTS_FILE" 2>/dev/null)
