...
---
# Implement 'justvibin remove' command

Implement the remove command for unregistering projects from justvibin.

## Usage

```
justvibin remove [name]      # Remove project from registry
justvibin remove --files     # Also delete project files (dangerous!)
```
...
cmd_remove() {
    ...
    if ! $delete_files && [[ -d "$project_path" ]]; then
        rm -f "$project_path/.justvibin"
        rm -f "$project_path/.justvibin.pid"
    fi
