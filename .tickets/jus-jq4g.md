...
---
# Implement 'justvibin tunnel' command
...
```
justvibin tunnel             # Quick tunnel for current project
justvibin tunnel <name>      # Quick tunnel for named project
justvibin tunnel --persist   # Persistent tunnel (requires config)
```
...
1. Requires ~/.config/justvibin/tunnel.toml:
```toml
[cloudflare]
...
cmd_tunnel() {
    ...
    if ! is_project_running "$project_name"; then
        ...
        log_info "Start first: justvibin start"
        exit 1
    fi
