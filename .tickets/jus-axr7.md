...
---
# Implement 'justvibin list' command
...
```
justvibin list           # List all projects
justvibin list --running # Only show running projects
justvibin list --json    # Output as JSON
```
...
cmd_list() {
    ...
    jq -r 'to_entries[] | "\(.key)|\(.value.port)|\(.value.path)|\(.value.template)"' \
        "$JUSTVIBIN_PROJECTS_FILE" | while IFS='|' read -r name port path template; do
    ...
    if [[ $total -eq 0 ]]; then
        ...
        log_info "Create one: justvibin new <name>"
    else
...
is_project_running() {
    local name="$1"
    local port=$(jq -r --arg n "$name" '.[$n].port' "$JUSTVIBIN_PROJECTS_FILE")
