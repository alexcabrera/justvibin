...
show_official_templates() {
    ...
    echo "    justvibin install https://github.com/alexcabrera/justvibin-with-django-and-hypermedia.git"
    echo ""
    echo "  hypertext"
    echo "    Vanilla HTML/CSS/JS with HTMX - simple static site"
    echo "    justvibin install https://github.com/alexcabrera/justvibin-with-hypertext.git"
    echo ""
}

offer_official_templates() {
    local templates=(
        "django-hypermedia|Django + HTMX + TailwindCSS|https://github.com/alexcabrera/justvibin-with-django-and-hypermedia.git"
        "hypertext|Vanilla HTML/CSS/JS + HTMX|https://github.com/alexcabrera/justvibin-with-hypertext.git"
    )
    
    for entry in "${templates[@]}"; do
        IFS='|' read -r name desc url <<< "$entry"
        
        if [[ -d "$JUSTVIBIN_TEMPLATES_DIR/$name" ]]; then
