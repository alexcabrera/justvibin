...
start_proxy_service() {
    ...
    launchctl bootstrap gui/$(id -u) "$JUSTVIBIN_PLIST"
...
stop_proxy_service() {
    ...
    launchctl bootout gui/$(id -u) "$JUSTVIBIN_PLIST" 2>/dev/null || true
...
is_proxy_running() {
    launchctl list 2>/dev/null | grep -q "$JUSTVIBIN_PROXY_LABEL"
}
```
