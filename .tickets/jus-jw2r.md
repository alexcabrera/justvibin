...
Migrate the 'justvibin setup' command from manual arg parsing to Cobra with comprehensive flags for headless operation.

## Background

justvibin is a CLI for scaffolding web projects. The 'setup' command performs first-time setup: dependency checks, config initialization, srv migration, Caddyfile generation, launchd proxy installation, CA trust, and official template installation.

## Current State

File: cmd/justvibin/setup.go
...
1. Create cmd/justvibin/cmd_setup.go with Cobra command:
...
var setupCmd = &cobra.Command{
    Use:   "setup",
    Short: "First-time setup for justvibin",
    Long:  "Initialize justvibin configuration, install dependencies, set up the HTTPS proxy service, and optionally install official templates.",
    RunE:  runSetupCmd,
}
...
```
Examples:
  justvibin setup                           # Interactive setup
  justvibin setup --check                   # Check dependencies only
  justvibin setup -y                        # Non-interactive, yes to all
  justvibin setup --skip-deps --no-migrate  # Headless, skip optional steps
  justvibin setup --templates django-hypermedia  # Install specific template
```

## Acceptance Criteria

- [ ] justvibin setup --help shows all flags with descriptions
- [ ] justvibin setup --check works as before
- [ ] justvibin setup -y runs without any prompts
- [ ] justvibin setup --install-deps installs missing deps
- [ ] justvibin setup --skip-deps fails if deps missing (no prompt)
- [ ] justvibin setup --migrate migrates without prompting
- [ ] justvibin setup --no-migrate skips migration without prompting
- [ ] justvibin setup --trust-ca trusts CA without prompting
- [ ] justvibin setup --no-trust-ca skips CA trust
- [ ] justvibin setup --install-templates installs all official
- [ ] justvibin setup --no-templates skips template installation
- [ ] justvibin setup --templates X installs only specified templates
