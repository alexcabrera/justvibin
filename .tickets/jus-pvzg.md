...
Implement interactive template selection: when user runs 'justvibin new' without --template and multiple templates exist, prompt to choose (huh select when TTY, fallback to numbered select). Handle non-interactive detection to avoid hanging (return error if stdin not tty). Integrate with template loader from config. Tests for fallback selection logic (non-interactive error, selection mapping).
