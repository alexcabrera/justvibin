...
tags: [ux, completion]
---
# Add shell completion for justvibin
...
_justvibin_completions() {
    ...
    case "$prev" in
        ...
        start|stop|open|tunnel|remove)
            ...
            local projects=$(jq -r 'keys[]' ~/.config/justvibin/projects.json 2>/dev/null)
            COMPREPLY=($(compgen -W "$projects" -- "$cur"))
            ;;
        --template|-t)
            # Complete with installed templates
            local templates=$(ls ~/.config/justvibin/templates/ 2>/dev/null)
...
complete -F _justvibin_completions justvibin
```
...
For bash completion, add to ~/.bashrc:
  source /opt/homebrew/etc/bash_completion.d/justvibin
