...
---
# Implement 'justvibin sync' command

Implement the sync command for rebuilding the project registry by scanning for .justvibin files.

## Usage

```
justvibin sync              # Scan home directory
justvibin sync <path>       # Scan specific path
...
cmd_sync() {
    ...
    log_info "Scanning for .justvibin files in $scan_path..."
    ...
    done < <(find "$scan_path" -name ".justvibin" -type f 2>/dev/null | grep -v "/.config/" | head -100)
...
sync_clean() {
    ...
    jq -r 'to_entries[] | "\(.key)|\(.value.path)"' "$JUSTVIBIN_PROJECTS_FILE" | \
    while IFS='|' read -r name path; do
        if [[ ! -d "$path" ]] || [[ ! -f "$path/.justvibin" ]]; then
