...
---
# Implement srv migration in justvibin setup

Implement migration logic for existing srv users in 'justvibin setup'.
...
if detect_srv_installation; then
    ...
    if confirm "Migrate srv projects to justvibin?"; then
        migrate_from_srv
    fi
fi
```
...
migrate_from_srv() {
    ...
    if [[ -f "$HOME/.config/srv/projects.json" ]]; then
        ...
        # For each srv project, create justvibin entry
        jq -r 'to_entries[] | "\(.key)|\(.value.port)|\(.value.path)"' "$srv_projects" | \
        while IFS='|' read -r name port path; do
            if [[ -d "$path" ]]; then
                # Convert .srv to .justvibin
                if [[ -f "$path/.srv" ]]; then
                    cat > "$path/.justvibin" << EOF
{
  "name": "$name",
  "template": "unknown",
  "port": $port,
...
- [ ] .srv files converted to .justvibin
