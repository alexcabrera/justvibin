...
---
# Test templates with new justvibin CLI

Comprehensive testing of both templates with the new unified justvibin CLI.
...
5. **List and open**
   ```bash
   justvibin list  # Shows testdjango
   justvibin open  # Opens browser
   ```
...
1. **Multiple projects simultaneously**
   ```bash
   justvibin start  # in testdjango
   justvibin start  # in testhtml (different terminal)
   justvibin list   # Both show running
   ```

2. **Tunnel test**
   ```bash
   justvibin tunnel  # Quick tunnel
   # Verify: public URL works
   ```

3. **Remove and cleanup**
   ```bash
   justvibin remove testdjango
   justvibin remove testhtml --files
   justvibin list  # Empty
   ```
