...
cmd_install() {
    ...
    if [[ -z "$url" ]]; then
        log_error "Usage: justvibin install <git-url>"
        exit 1
    fi
    ...
    if ! validate_manifest "$tmp_dir/justvibin.toml"; then
        rm -rf "$tmp_dir"
        exit 1
    fi
    ...
    local desc=$(parse_toml_value "$target/justvibin.toml" "template.description")
    log_info "$desc"
}
```
