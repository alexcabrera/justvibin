...
Migrate the 'justvibin proxy' command from manual arg parsing to Cobra with proper subcommands.

## Background

justvibin is a CLI for scaffolding web projects. The 'proxy' command manages a local Caddy HTTPS proxy service via launchd.

## Current State

File: cmd/justvibin/proxy.go
...
1. Create cmd/justvibin/cmd_proxy.go with Cobra command and subcommands:
...
- [ ] justvibin proxy --help shows available subcommands
- [ ] justvibin proxy start/stop/restart/status/logs work
- [ ] justvibin proxy status --json outputs valid JSON
- [ ] justvibin proxy logs -n 50 shows last 50 lines
- [ ] justvibin proxy logs -f=false shows logs without following
