...
func TestInstallProxyServiceSkipsWhenRunning(t *testing.T) {
	...
	if err := InstallProxyService(ctx, runner, "/tmp/justvibin.plist"); err != nil {
		t.Fatalf("install: %v", err)
	}
	for _, call := range runner.calls {
...
func TestUninstallProxyServiceSkipsWhenStopped(t *testing.T) {
	...
	if err := UninstallProxyService(ctx, runner, "/tmp/justvibin.plist"); err != nil {
		t.Fatalf("uninstall: %v", err)
	}
	for _, call := range runner.calls {
