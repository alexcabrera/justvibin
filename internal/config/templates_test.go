...
func TestDefaultTemplates(t *testing.T) {
	...
	if len(got.Ordered) != 2 {
		t.Fatalf("expected 2 defaults, got %d", len(got.Ordered))
	}
	if got.Ordered[0].Name != "django-hypermedia" || got.Ordered[1].Name != "hypertext" {
		t.Fatalf("unexpected ordering: %#v", got.Ordered)
	}
	if got.ByName["django-hypermedia"].URL == "" {
		t.Fatalf("expected django-hypermedia url")
	}
}
...
func TestLoadTemplatesOverridesAndAppend(t *testing.T) {
	...
	content := strings.Join([]string{
		"[templates]",
		"django-hypermedia = \"https://override.example/repo.git\"",
	...
	if got.ByName["django-hypermedia"].URL != "https://override.example/repo.git" {
